...
res_dt <- fread(paste0(home, "/Analysis/outputs/MODIS_1Y/meanResiduals.csv"))
et_dt <- fread(paste0(home, "/Analysis/outputs/MODIS_1Y/meanET.csv"))
spi_dt <- fread(paste0(home, "/Analysis/outputs/MODIS_1Y/meanSPI.csv"))
...
meanResid <- DTtoRast(meanResid_dt, ExampleRaster, paste0(home, "/Analysis/outputs/MODIS_1Y/compositeRasters/composite_residuals.tif"), open = T)

# now put the mean ET anomalies into a raster for comparison 
sel_cols <- c("cellnum", "meanET")
meanET_dt <- et_dt[, ..sel_cols]
meanET <- DTtoRast(meanET_dt, ExampleRaster, paste0(home, "/Analysis/outputs/MODIS_1Y/compositeRasters/composite_ETanom.tif"), open = T)
...
for(i in 1:length(lowers)){
  ...
  et_comps <- stack(et_comps, DTtoRast(out, ExampleRaster, paste0(home, "/Analysis/outputs/MODIS_1Y/compositeRasters/ETanom_", names(out)[2], ".tif"), open=T))
}
...
for(i in 1:length(lowers)){
  ...
  res_comps <- stack(res_comps, DTtoRast(out, ExampleRaster, paste0(home, "/Analysis/outputs/MODIS_1Y/compositeRasters/residual_", names(out)[2], ".tif"), open=T))
}
...
noDrought <- fread(paste0(home, "/Analysis/outputs/MODIS_1Y/avgNonDroughtETResponse.csv"))
...
fwrite(data, paste0(home, "/Analysis/outputs/MODIS_1Y/final_drought_attribute_dt.csv"))
