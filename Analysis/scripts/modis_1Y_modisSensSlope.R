...
res_dt_long <- gather(res_dt[,1:(ncol(res_dt)-1)], Year, ET_Residual, `2000`:`2017`)
et_dt_long <- gather(et_dt[,1:(ncol(et_dt)-1)], Year, ET_anom, `2000`:`2017`)
spi_dt_long <- gather(spi_dt[,1:(ncol(spi_dt)-1)], Year, SPI, `2000`:`2017`)
...
dr_trend_rast <- DTtoRast(sig_trend, ExampleRaster, paste0(home, "/Analysis/outputs/MODIS_1Y/sig_drought_trend.tif"), open = T)
