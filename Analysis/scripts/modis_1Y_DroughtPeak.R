...
for(i in 1:length(years)){
  ...
  droughtID(spi_sub, et_sub, n, home, y, "MODIS_1Y")
  print(i)
}
...
modFit(files_dt_gs$spi_file, files_dt_gs$et_file, fmask, n, "MODIS_1Y")
...
peak_files <- list.files(paste0(home, "/Analysis/outputs/MODIS_1Y/droughtPeak"), full.names = T, pattern = ".tif$")
...
beta_file <- paste0(home, "/Analysis/outputs/MODIS_1Y/model_beta_coef.csv")
...
foreach(i = 1:nrow(peak_dt))%dopar%{
  ...
  y <- peak_dt$date[i]
  calcResiduals(spi_file, et_file, beta_file, y, "MODIS_1Y")
}
...
# residual files 
resid_files <- list.files(paste0(home, "/Analysis/outputs/MODIS_1Y/residuals"), full.names = T, pattern = ".csv$")
avgResid(resid_files, n, "MODIS_1Y")

# now do ET 
yrET_files <- list.files(paste0(home, "/Analysis/outputs/MODIS_1Y/ETanom"), full.names = T, pattern = ".csv$")
avgET(yrET_files, n, "MODIS_1Y")

# now SPI 
yrSPI_files <- list.files(paste0(home, "/Analysis/outputs/MODIS_1Y/spi"), full.names = T, pattern = ".csv$")
avgSPI(yrSPI_files, n, "MODIS_1Y")
