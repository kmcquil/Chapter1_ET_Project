################################################################################################
## Process downloaded landsat ET into a usable format 
### tarred files to .tif 
### scale ETa by 0.001
### mask out pixels that are not high quality 
### crop to region, reproject to 32616
### mosaic together images from the same day to a common template that spans full ROI 
### crop and mask to the ROI boundary 
################################################################################################# 
library(raster)
library(data.table)
library(rgdal)
...
print(cl) # see how many cores are being used 

home <- "/share/klmarti3/kmcquil/Chapter1_ET_Project/Data" # set a home directory
roi_mask <- readOGR( paste0(home, "/NA_CEC_Eco_Level3/blue_ridge.shp")) # ROI shapefile 

##############################################################################################################
# make a list of the tar.gz files
...
foreach(i = 1:length(files))%dopar%{
  ...
  # untar the file and save to new directory 
...
########################################################################################################
# start a new cluster that uses less cores since this part is more memory intensive 
UseCores <- 20
cl <- makeCluster(UseCores)
registerDoParallel(cl)
